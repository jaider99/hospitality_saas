...
SYSTEM_PROMPT = (
    ...
    "META INVOICE PAID STATUS: If the invoice shows 'Pagado' (or 'PAGADO'), set paidStatus = 'paid'. The document title may say 'Comprobante de pago/nota de pago pendiente' — this is just the DOCUMENT TYPE label, not the payment status. Always look at the explicit 'Pagado' stamp to determine paidStatus.\n"
    "DIGITAL AD INVOICE SUPPLIER: The supplier is the ad platform. The customer is the advertiser. Do not swap their VAT IDs. The ad platform's VAT ID is usually in the header or footer.\n"
    ...
    "  • EXPLICIT DISCOUNTS: If an item has a printed discount (e.g., under 'DTOS' or clumped as '40.59-16,2424.35'), extract its absolute value as a positive number into `appliedDiscount`. If you see '-16,24', split it logically and set appliedDiscount: 16.24.\n"
    ...
    "ALSO CRITICAL: Sub-rows that describe the ad set detail (e.g., 'Farola Marzo reconocimiento de marca - 15.081 Impresiones €10,03') are the SAME line item as the campaign header above them with the matching price. NEVER extract the sub-row as a third separate item — there must be EXACTLY ONE item per campaign, not two. The word 'Impresiones' (impressions) is the metric count, NOT a product. Do NOT create a product called 'Farola Marzo reconocimiento de marca' if you already extracted 'Farola reconocimiento de marca MAYO' for the same price.\n"
    "\n"
    ...
    "MARGINAL TEXT RULE: Often the supplier's legal name and VAT ID are printed in tiny text at the very edge/margin of the receipt (e.g., 'Registre Mercantil... C.I.F. - A 08064313'). Always check the very beginning or end of the raw text (or edges of the image) for this information.\n"
    ...
    "TWO-COLUMN RULE: The OCR text may contain explicit column markers like '[LEFT COLUMN]' and '[RIGHT COLUMN]' followed by '[END COLUMNS]'. These markers simply denote that text was printed side-by-side on the page.\n"
    "  • CONTEXT MATTERS: If these markers appear in the header/footer with addresses, they usually separate the Supplier from the Customer (e.g. Left = Supplier, Right = Customer). Extract supplier fields ONLY from the supplier's block.\n"
    "  • If these markers appear above the items table (e.g. Left = 'CONCEPTO', Right = 'Cantidad Precio...'), they are just table headers. Treat them normally as part of the table.\n"
    "  • Example: '[LEFT COLUMN]' lists 'Supplier Company A / Address A / VAT A'. '[RIGHT COLUMN]' lists 'Customer Company B / Address B / VAT B'. In this case, supplier.name = 'Supplier Company A', supplier.vatID = 'VAT A', supplier.address = 'Address A'. Do NOT mix the two columns!\n"
    "\n"
    "=== CORRUPTED DATES ===\n"
