...
def upgrade() -> None:
    ...
    # Clean up invalid owner_id references to prevent foreign key violation
    if 'user' in tables and 'restaurant' in tables:
        op.execute('UPDATE restaurant SET owner_id = NULL WHERE owner_id NOT IN (SELECT id FROM "user")')
