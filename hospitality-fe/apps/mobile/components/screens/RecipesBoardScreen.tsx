...
export default function RecipesBoardScreen() {
  ...
  const handleDeleteTag = async (tagId: string) => {
    try {
      ...
      setAlertConfig({
        ...
        message: 'This category cannot be deleted because it is currently assigned to one or more recipes.'
      });
    } finally {
      setTagToDelete(null);
    }
  ...
  return (
    <View style={{ flex: 1, gap: 14 }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <TouchableOpacity
            ...
            style={{ borderWidth: 1, borderColor: '#e2e1dd', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 8, backgroundColor: '#ffffff' }}
          >
            <Text style={{ fontSize: 11, fontWeight: '600', color: '#151515', fontFamily: 'Sora' }}>Manage Types</Text>
          </TouchableOpacity>
          <TouchableOpacity
      ...
      {/* Tabs */}
      <View style={{ flexDirection: 'row', borderBottomWidth: 1, borderBottomColor: '#e2e1dd', paddingBottom: 1 }}>
        <TouchableOpacity
          onPress={() => { setActiveTab('dishes'); setSelectedTagFilter(''); }}
          style={{ flex: 1, alignItems: 'center', paddingVertical: 12, borderBottomWidth: activeTab === 'dishes' ? 2 : 0, borderBottomColor: '#151515' }}
        >
          <Text style={{ fontSize: 13, fontWeight: activeTab === 'dishes' ? '700' : '500', color: activeTab === 'dishes' ? '#151515' : '#8c8c89', fontFamily: 'Sora' }}>
            Recipes & Drinks
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => { setActiveTab('preparations'); setSelectedTagFilter(''); }}
          style={{ flex: 1, alignItems: 'center', paddingVertical: 12, borderBottomWidth: activeTab === 'preparations' ? 2 : 0, borderBottomColor: '#151515' }}
        >
          <Text style={{ fontSize: 13, fontWeight: activeTab === 'preparations' ? '700' : '500', color: activeTab === 'preparations' ? '#151515' : '#8c8c89', fontFamily: 'Sora' }}>
            Preparations
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={{ flexDirection: 'row', gap: 10, alignItems: 'center' }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6 }}>
          ...
          >
            <Text style={{ fontSize: 12, fontWeight: '600', color: !selectedTagFilter ? '#ffffff' : '#8c8c89', fontFamily: 'Sora' }}>
              All Types
            </Text>
          </TouchableOpacity>
          {recipeTags
            ...
            .map((tag) => (
              ...
              >
                <Text style={{ fontSize: 12, fontWeight: '600', color: selectedTagFilter === tag.name ? '#ffffff' : '#8c8c89', fontFamily: 'Sora' }}>
                  {tag.name}
                </Text>
              </TouchableOpacity>
            ))}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            const costPct = item.margin ? (1 - item.margin) * 100 : (salePrice > 0 ? (cost / salePrice) * 100 : 0);
            const labelVariant = getStatusVariant(costPct);

            return (
              <TouchableOpacity
                onPress={() => router.push({ pathname: '/recipes/[id]', params: { id: item.dbId || item.id } })}
                style={{
                  backgroundColor: '#ffffff',
                  borderWidth: 1,
                  borderColor: '#e2e1dd',
              ...
              >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <View style={{ flex: 1, marginRight: 8 }}>
                    <Text numberOfLines={1} style={{ fontSize: 14, fontWeight: '700', color: '#151515', fontFamily: 'Sora' }}>
                      {item.name}
                    </Text>
                    {item.tag?.name && (
                      <Text style={{ fontSize: 10, color: '#8c8c89', fontFamily: 'Sora', marginTop: 2 }}>
                        {activeTab === 'preparations' ? 'Preparation type' : 'Dish type'}: {item.tag.name}
                      </Text>
                    )}
                  </View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <Badge
                      label={labelVariant === 'danger' ? 'Alert' : labelVariant === 'warning' ? 'Warning' : 'Stable'}
                      variant={labelVariant}
                    />
                    <TouchableOpacity onPress={() => setRecipeToDelete(item)} style={{ padding: 4 }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', borderTopWidth: 1, borderTopColor: '#f1f0ec', paddingTop: 8 }}>
                  <View>
                    <Text style={{ fontSize: 9, color: '#8c8c89', fontFamily: 'Sora', textTransform: 'uppercase' }}>GRP</Text>
                    <Text style={{ fontSize: 12, fontWeight: '700', color: '#151515', fontFamily: 'DM Mono' }}>
                      €{Number(salePrice).toFixed(2)}
                    </Text>
                  </View>
                  <View>
                    <Text style={{ fontSize: 9, color: '#8c8c89', fontFamily: 'Sora', textTransform: 'uppercase' }}>Cost</Text>
                    <Text style={{ fontSize: 12, fontWeight: '700', color: '#151515', fontFamily: 'DM Mono' }}>
                      €{Number(cost).toFixed(2)}
                    </Text>
                  </View>
                  <View>
                    <Text style={{ fontSize: 9, color: '#8c8c89', fontFamily: 'Sora', textTransform: 'uppercase' }}>Margin</Text>
                    <Text style={{ fontSize: 12, fontWeight: '700', color: '#1f8f5c', fontFamily: 'DM Mono' }}>
          ...
          ListEmptyComponent={() => (
            <View style={{ padding: 32, alignItems: 'center', gap: 10, marginTop: 20 }}>
              ...
              <Text style={{ fontSize: 13, fontWeight: '600', color: '#151515', fontFamily: 'Sora' }}>No recipes found</Text>
            </View>
          )}
        />
      )}
      ...
      <Modal visible={isCreateOpen} animationType="slide">
        <View style={styles.modalContainer}>
          ...
          <ScrollView contentContainerStyle={styles.modalBody}>
            <Input label="Recipe Name" value={newRecipeName} onChangeText={setNewRecipeName} placeholder="e.g. Negroni" />
            <Input label="Unit of Measure" value={newRecipeUom} onChangeText={setNewRecipeUom} placeholder="ud, L, kg" />
            
            <Text style={styles.inputLabel}>{activeTab === 'preparations' ? 'Preparation Type' : 'Dish Type'}</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, marginVertical: 6 }}>
              {recipeTags
                ...
                .map((tag) => (
                  ...
                  >
                    <Text style={{ fontSize: 12, fontWeight: '600', color: newRecipeTagName === tag.name ? '#ffffff' : '#151515', fontFamily: 'Sora' }}>
                      {tag.name}
                    </Text>
                  </TouchableOpacity>
                ))}
